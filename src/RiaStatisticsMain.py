...
beforeUrl = "https://wiki.ria.red/wiki/%E5%AE%A1%E6%A0%B8%E8%AE%B0%E5%BD%95%E9%A1%B5(202202~202209)"
def askURL(url):
...
def get_results_list(url = baseurl):
    html = askURL(url)
...
def info_st(results_list=None) -> dict:
    if results_list is None:
        results_list = get_results_list()
    results_dict = {}
    for result in results_list:
        try:
            ...
            continue
        small = re.findall(r'(<small>(.*?)</small>)', result[3])
        sub = re.findall(r'(<sub>(.*?)</sub>)', result[3])
        try:
            if small != []: # do not believe compiler
                small = ''.join(small[0][0])
                name = result[3].replace(small, "")
            elif sub != []:
                sub = ''.join(sub[0][0])
...
def sub_st(results_list=None) -> dict:
    if results_list is None:
        results_list = get_results_list()
    results_dict = {}
    for result in results_list:
...
show_pie(test_st(), "Ria测试统计")
show_pie(info_st(), "Ria备注信息统计")
show_pie(sub_st(), "Ria小标题统计")

show_pie(op_st(beforeUrl), "往年Ria检票员活跃数统计")
results_list = get_results_list(beforeUrl)
show_pie(test_st(results_list), "往年Ria测试统计")
#show_pie(info_st(results_list), "往年Ria备注信息统计") # bugs here but i don't wanna fix it
show_pie(sub_st(results_list), "往年Ria小标题统计")
